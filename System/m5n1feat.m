function [feat] = m5n1feat(eegraw)
%   DWT, db4 for L3, cD3, then Energy Entropy 14 channel EEG data
%   Detailed explanation goes here
    ...
    %Feature EXTRACTION
     for ch_itr = channels
        fv_itr = fv_itr + 1;
        f1(fv_itr) = sum((abs(cD3(:,ch_itr))).^2); %energy
        f2(fv_itr) = -sum(((abs(cD3(:,ch_itr))).^2).*((log(abs(cD3(:,ch_itr))).^2))); %entropy
    end
    %until here
